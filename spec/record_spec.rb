...
RSpec.describe RegistersClient::Record do
  let(:item_rsf_line) {"add-item\t{\"citizen-names\":\"Briton;British citizen\",\"country\":\"GB\",\"name\":\"United Kingdom\"}"}

  describe 'item' do
    it 'should return the item that the record initialized with' do
      expected_item = RegistersClient::Item.new(item_rsf_line)
      record = RegistersClient::Record.new(RegistersClient::Entry.new("", 1, :user), expected_item)
  ...
  describe 'entry' do
    it 'should return the entry that the record was initialized with' do
      ...
      expected_entry = RegistersClient::Entry.new(rsf_line, 1, :user)
